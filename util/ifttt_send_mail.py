...
- 아래의 구조로 내용 작성
    ...
    url    , {여기에 ifttt webhook url 링크}   # 예: https://maker.ifttt.com/trigger/{event}/with/key/{my_key}
                                            # 참고: ~~/json/with/key~~ 가 아니라 ~~/with/key~~ 임
                                            #     json이 안되어서 'application/x-www-form-urlencoded'을 사용함
"""
...
def send_email_via_ifttt(text, send_to='sample@sample.com'):
    """ 파이썬 -> webhook -> ifttt -> mail 발송

    Args:
        text: 메일로 전송할 내용, 한 문장만 아주 짧게 작성
        send_to: 메일을 전송할 이메일 주소
