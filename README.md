...
2. 기본 설정으로 실행할 샘플 엑셀파일(.xlsx)을 `test/input/data.xlsx`에 둡니다.
3. 상단의 `_setting()` 에서 세부 설정을 조정할 수 있음
4. 사용자 정의 불용어가 필요하면 `input/stopwordlist.example.txt`를 참고해 `test/input/stopwordlist.txt`를 작성합니다.
5. `preprocessing.py`는 `test/input/data.xlsx`에 `preprocessed` 시트를 추가합니다.
6. 빈도분석, LDA, 시계열 분석 결과물은 `test/output/`에 저장됩니다. 필요한 출력 폴더는 자동으로 생성됩니다.
7. `input/`, `output/`은 추후 실제 사용자 실행 흐름에서 별도로 사용할 예정입니다.
